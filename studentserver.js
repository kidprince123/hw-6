...
function readStudents(files, studentList, res, lastname) {
  if (files.length === 0){
    //done reading all the students into array
    //now filter for students with specific lastname

    const filteredStudents = studentList.filter(student => student.last_name === lastname);
    if(filteredStudents.length === 0){
      return res.status(404).send({"message": "No student found with specified last name."});
    }
    var obj ={};
...
/**
 * @swagger
 * /students/search/lastname/{lastname}:
 *   get:
 *     summary: Get a student by lastname.
 *     description: Use this endpoint to retrieve a student based on their lastname..
 *     parameters:
 *       - name: lastname
...
app.get('/students/search/:lastname', function (req, res) {
  ...
  glob("students/*.json", null, function (err, files) {
    ...
    // Call the correct function name: readStudents
    readStudents(files, arr, res, lastname);
